...
def assign_tone(text: str) -> str:
    ...
    if re.search(
        r"мер достаточно|достаточно мер|доступ есть|вс[её] устраивает|вс[eё] нормально|нет проблем|"
        r"не требуется|всего вполне|всем вс[её] доступно|вс[eё] есть|принимаются достаточные",
        low,
    ):
    ...
    if re.search(
        ...
        r"не работает|искусственн|не поможet|ничего не поможет|руки опуск",
        low,
    ):
